...
<div x-data="surveyApp()" x-cloak>
    {{-- ── Email Gate ── --}}
    <template x-if="!respondentEmail">
        <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-lg shadow-sm p-12 text-center max-w-md w-full">
                <h1 class="text-2xl font-bold mb-2 text-[#003087]">ADDU Alumni Tracer Study</h1>
                <p class="text-gray-600 mb-6">Please enter your email to continue</p>
                <div class="space-y-3">
                    <input
                        type="email"
                        class="w-full px-4 py-3 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#003087]"
                        placeholder="your.email@example.com"
                        x-model="emailInput"
                        @keydown.enter="checkEmail()"
                    >
                    <template x-if="emailError">
                        <p class="text-sm text-red-600" x-text="emailError"></p>
                    </template>
                    <button
                        @click="checkEmail()"
                        :disabled="emailChecking"
                        class="w-full px-6 py-3 bg-[#003087] text-white rounded-lg font-medium hover:bg-[#002366] transition-colors disabled:opacity-50"
                    >
                        <span x-text="emailChecking ? 'Checking...' : 'Continue'"></span>
                    </button>
                </div>
                <div class="mt-6 pt-4 border-t border-gray-200 text-center">
                    <a href="/admin" class="text-sm text-[#003087] hover:underline">Admin Panel →</a>
                </div>
            </div>
        </div>
    </template>
                ...
                <div class="max-w-4xl mx-auto px-6 py-4">
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-muted-foreground mt-1">
                                <template x-if="isEditMode">
                                    <span x-text="'Editing responses for ' + respondentEmail"></span>
                                </template>
                                <template x-if="!isEditMode">
                        ...
                        <div class="w-full max-w-xs bg-gray-200 rounded-full h-2">
                            <div
                                class="bg-[#003087] h-2 rounded-full transition-all duration-300"
                                                ...
                                                <template x-for="answer in sortedAnswers(question)" :key="answer.id">
                                                    <label class="flex items-center gap-2 cursor-pointer">
                                                        <input
                                                            ...
                                                            :checked="formData[question.id] === answer.text || (answer.text.toLowerCase().includes('other') && (formData[question.id] || '').startsWith(answer.text + ': '))"
                                                            @change="formData[question.id] = answer.text"
                                                        >
                                                        <span class="text-sm" x-text="answer.text"></span>
                                                    </label>
                                                </template>
                                                {{-- Others specify text box --}}
                                                <template x-if="sortedAnswers(question).some(a => a.text.toLowerCase().includes('other')) && (formData[question.id] || '').toLowerCase().startsWith('others')">
                                                    <div class="col-span-full mt-1">
                                                        <input
                                                            ...
                                                            @input="formData[question.id] = getOthersLabel(question) + ': ' + $event.target.value"
                                                        >
                                                    </div>
                                                </template>
                                            </div>
...
function surveyApp() {
    ...
    return {
        ...
        async checkEmail() {
            ...
            if (!email) { this.emailError = 'Please enter your email address.'; return; }
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { this.emailError = 'Please enter a valid email address.'; return; }

            this.emailChecking = true;
            this.emailError = '';

            try {
                const res = await fetch('/survey/check-email', {
                    method: 'POST',
